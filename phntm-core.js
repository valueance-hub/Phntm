    ...
    _start() {
      ...
      const draw = () => {
        ...
        for (let v = gStart; v <= yMax; v += step) {
          ...
          ctx.fillStyle = '#5c5c5c'; ctx.fillText(kLabel(v), padL - 10, yy);
        }
        ...
        ctx.fillText('EQUITY ($)', x0 - 2, topY - 12);
        ...
        if (n > 1) {
          ...
          if (dayKey(PTS[0].ts) === dayKey(PTS[n - 1].ts)) {
            ctx.fillStyle = '#5c5c5c'; ctx.textAlign = 'left'; ctx.fillText(tLabel(PTS[0].ts), x0, botY + 18);
            ctx.textAlign = 'right'; ctx.fillText(tLabel(PTS[n - 1].ts), x0 + plotW, botY + 18);
            ctx.fillStyle = '#4c4c4c'; ctx.textAlign = 'center'; ctx.fillText(dLabel(PTS[0].ts), x0 + plotW / 2, botY + 18);
          } else {
            let prev = null;
            for (let i = 0; i < n; i++) { const k = dayKey(PTS[i].ts); if (k === prev) continue; prev = k; const xx = x(i); ctx.strokeStyle = 'rgba(255,255,255,.04)'; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(xx, topY); ctx.lineTo(xx, botY); ctx.stroke(); ctx.fillStyle = '#5c5c5c'; ctx.textAlign = i === 0 ? 'left' : (i === n - 1 ? 'right' : 'center'); ctx.fillText(dLabel(PTS[i].ts), xx, botY + 18); }
          }
          ctx.textBaseline = 'middle';
